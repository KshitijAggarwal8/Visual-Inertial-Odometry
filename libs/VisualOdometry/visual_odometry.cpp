...
vo::VisualOdometry::VisualOdometry(Eigen::Matrix4d initial_pose) {
    ...
    // [0, 0, 1]
    camera_intrinsics = (cv::Mat_<double>(3, 3) << 172.98992850734132, 0, 163.33639726024606, 0, 172.98303181090185, 134.99537889030861, 0, 0, 1);

    // Initialize distortion coefficients (Taken from dataset)
    // [-0.027576733308582076, -0.006593578674675004, 0.0008566938165177085, -0.00030899587045247486]
    distortion_coefficients = (cv::Mat_<double>(1, 4) << -0.027576733308582076, -0.006593578674675004, 0.0008566938165177085, -0.00030899587045247486);
    ...
    new_camera_matrix = cv::getOptimalNewCameraMatrix(camera_intrinsics, distortion_coefficients, cv::Size(image_width, image_height), 1, cv::Size(image_width, image_height), 0);
 ...
 * 
 */
Eigen::Matrix4d vo::VisualOdometry::get_pose() {
    return vo_pose;
}
...
void vo::VisualOdometry::update_pose(cv::Mat image) {
    ...
    cv::undistort(image, undistorted_image, camera_intrinsics, distortion_coefficients, new_camera_matrix);

    // Get keypoints and descriptors for the current image
    orb_descriptor->detectAndCompute(undistorted_image, cv::noArray(), kp_curr, des_curr);
    ...
    cv::Mat E = cv::findEssentialMat(matched_kp_curr, matched_kp_prev, camera_intrinsics, cv::RANSAC, 0.999, 1.0);
