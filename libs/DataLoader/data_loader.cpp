 ...
 */
std::tuple<long double, Eigen::Vector3d, Eigen::Vector3d> dl::DataLoader::get_imu_data() {
    if (imu_file.eof())
    ...
    if (std::getline(imu_file, line)) {
        ...
        if (iss >> id >> timestamp >> ang_vel_x >> ang_vel_y >> ang_vel_z >> lin_acc_x >> lin_acc_y >> lin_acc_z) {
...
void dl::DataLoader::parse_gt_data() {
    ...
    Eigen::Quaterniond initial_quaternion_orientation(qw_gt[0], qx_gt[0], qy_gt[0], qz_gt[0]);

    // Make the quaternion a rotation matrix
    Eigen::Matrix3d initial_rotation_matrix = initial_quaternion_orientation.toRotationMatrix();

    // Un-rotate the x, y, z values
    for (size_t i = 0; i < x_gt.size(); ++i) {
        Eigen::Vector3d point(x_gt[i], y_gt[i], z_gt[i]);
        Eigen::Vector3d unrotated_point = initial_rotation_matrix.transpose() * point;
    ...
    for (size_t i = 0; i < qx_gt.size(); ++i) {
        ...
        Eigen::Matrix3d unrotated_orientation = initial_rotation_matrix.transpose() * rotation_matrix;
